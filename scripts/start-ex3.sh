...
bun run src/node/reviews/reviewProcessor.ts  > "$LOG_DIR/review-processor.log"   2>&1 &
bun run src/node/reviews/reviewAnalytics.ts  > "$LOG_DIR/review-analytics.log"   2>&1 &

echo "Exercise 3 services started. Logs in $LOG_DIR/"
echo "Start the producer manually: bun run src/node/reviews/reviewProducer.ts"
